...
export const glyphEffects = {
  ...
  dilationgalaxyThreshold: {
    ...
    effect: (level, strength) => Math.clampMin(1 - Math.pow(level, 0.17) * Math.pow(strength, 0.35) / 100 -
      GlyphAlteration.sacrificeBoost("dilation") / 50, 0.0001),
      // Hardcap the effect to 0.0001x to prevent negative TG threshold, causing a game crash
    formatEffect: x => format(x, 3, 3),
