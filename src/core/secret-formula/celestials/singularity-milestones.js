...
export const singularityMilestones = {
  ...
  multFromInfinitied: {
    ...
    effect: () => Math.clampMax(Math.clampMin(Currency.infinitiesTotal.value.pLog10() / 1000, 1), 1e10),
    effectFormat: x => formatX(x, 2, 2),
    upgradeDirection: LAITELA_UPGRADE_DIRECTION.BOOSTS_LAITELA,
  },
  ...
  darkFromGlyphLevel: {
    ...
    effect: () => Math.clampMax(Math.pow(Math.clampMin((player.records.bestReality.glyphLevel - 15000) / 2000, 1), 0.5), 1e10),
    effectFormat: x => formatX(x, 2, 2),
    upgradeDirection: LAITELA_UPGRADE_DIRECTION.BOOSTS_LAITELA,
  },
  ...
  darkFromTheorems: {
    ...
    effect: () => Math.clampMax(Math.sqrt(Math.clampMin((Currency.timeTheorems.value.log10() - 1000) / 50, 1)), 1e10),
    effectFormat: x => formatX(x, 2, 2),
    upgradeDirection: LAITELA_UPGRADE_DIRECTION.BOOSTS_LAITELA,
  },
  dim4Generation: {
  ...
  darkFromDilatedTime: {
    ...
    // Hardcap this to prevent instant game crashes. A 1e50 DM bonus from DT is impossible normally anyways
    effect: () => Math.clampMax(Math.pow(1.6, Decimal.log10(Currency.dilatedTime.value.plus(1)) / 1000), 1e50),
    effectFormat: x => formatX(x, 2, 2),
    upgradeDirection: LAITELA_UPGRADE_DIRECTION.BOOSTS_LAITELA,
  },
  infinitiedPow: {
