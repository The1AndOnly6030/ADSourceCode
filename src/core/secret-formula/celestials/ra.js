export const ra = {
  pets: {
    teresa: {
      ...
      rawMemoryChunksPerSecond: () => 4 * Math.pow(Currency.eternityPoints.value.pLog10() / 1e4, 15),
    ...
    effarig: {
      ...
      rawMemoryChunksPerSecond: () => 4 * Math.pow(Effarig.shardsGained, 0.5),
    ...
    enslaved: {
      ...
      rawMemoryChunksPerSecond: () => 4 * Math.pow(Currency.timeShards.value.pLog10() / 3e5, 10),
    ...
    v: {
      ...
      rawMemoryChunksPerSecond: () => 4 * Math.pow(Currency.infinityPower.value.pLog10() / 1e7, 7.5),
