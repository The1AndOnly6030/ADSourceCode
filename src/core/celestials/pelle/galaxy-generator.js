...
export const GalaxyGenerator = {
  ...
  loop(diff) {
    ...
    if (this.sacrificeActive) {
      this.capRift.reducedTo = Math.max(this.capRift.reducedTo - 0.1 * diff / 1000, 0);
    ...
    if (!this.capRift) {
      PelleRifts.all.forEach(r => r.reducedTo = Math.min(r.reducedTo + (0.01 * Math.log10(player.celestials.pelle.galaxyGenerator.generatedGalaxies)) * diff / 1000, 9.99999));
